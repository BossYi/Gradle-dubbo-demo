...
import priv.yimeng.common.core.Filter.Operator;
...
public class Pageable implements Serializable {
     ...
     * <p>
     * <pre>
     * 只有配置了querySelectName参数才会生效，且实体上对应构造函数的第一个参数会对应count值
     * <pre>
     *
     * @param groupBy
     *            是否需要分组
    ...
    @Override
    public int hashCode() {
        return new HashCodeBuilder(17, 37).append(getPageNumber()).append(getPageSize()).append(getSearchProperty()).append(getSearchValue())
                .append(getOrderProperty()).append(getOrderDirection()).append(getFilters()).append(getOrders()).toHashCode();
    }
